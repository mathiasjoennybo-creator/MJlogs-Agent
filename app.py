...
with fane_indstillinger:
    ...
    if ai_klar:
        st.success("✅ AI Hjerne (Gemini) Aktiv")
    else:
        st.error("❌ AI Hjerne Mangler")
        
    if sms_klar:
        st.success("✅ SMS Modul (Twilio) Aktiv")
    else:
        st.warning("⚠️ SMS Modul i Test-Tilstand")
